...
describe('common/Floorplan/FloorplanRange.vue', () => {
  ...
  it('位置產生自 currentPanorama 的 position', () => {
    ...
    expect(vm.$el.style['-webkit-transform'])
      .to.equal(`translate(${x}px, ${y}px)`)
  })
  ...
  it('ratioW、floorplanRatioX、xOffset 會協助修正扇形的位置', () => {
    ...
    expect(vm.$el.style['-webkit-transform'])
      .to.equal(`translate(${x}px, ${store.state.panoramas.currentPanorama.position.y}px)`)
  })
  ...
  it('ratioH、floorplanRatioY、yOffset 會協助修正扇形的位置', () => {
    ...
    expect(vm.$el.style['-webkit-transform'])
      .to.equal(`translate(${store.state.panoramas.currentPanorama.position.x}px, ${y}px)`)
  })
  ...
  it('角度產生自 currentPanorama 的 floorplanRotation，krpanoCamera.krpanoLookAt.h、krpanoXOffset、currentPanorama.panoramaRotation.y 會影響結果', () => {
    ...
    expect(rangeEl.style['-webkit-transform'])
      .to.equal(`rotateZ(${floorplanRotation + cameraMoving}deg)`)
  })
})
