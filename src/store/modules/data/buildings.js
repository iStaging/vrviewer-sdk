...
export const actions = {
  async fetchBuildings ({ dispatch, commit, rootState }, userId = '') {
    const fetchBuildings = await firebase.database().ref('/buildings').orderByChild('Owner').equalTo(userId)
    fetchBuildings[process.env.USE_FULL_SYNC ? 'on' : 'once']('value', snapshot => {
  ...
  async fetchBuilding ({ dispatch, commit, state, rootState }, buildingId = '') { // should have some value to return data null
    if (buildingId) {
      ...
      if (process.env.USE_FULL_SYNC) {
        if (!isEmpty(state.currentBuilding)) {
          const oldBuildingId = state.currentBuilding.objectId
          const oldFetchBuilding = await firebase.database().ref(`/buildings/${oldBuildingId}`)
          oldFetchBuilding.off()
        }
      }
      const fetchBuilding = await firebase.database().ref(`/buildings/${buildingId}`)
      fetchBuilding[process.env.USE_FULL_SYNC ? 'on' : 'once']('value', async snapshot => {
        const resp = snapshot.val()
  ...
  selectBuilding ({ state, rootState }, building = {}) {
    ...
    gaEvents.sendEvent('Property', 'ChangeBuilding', propertyId)
