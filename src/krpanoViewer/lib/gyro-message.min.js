...
var krpanoplugin = function () {
  ...
  local.registerplugin = function (krpanointerface, pluginpath, pluginobject) {
    ...
    if (isTopAccessible === undefined) {
      ...
      isTopAccessible = false;
      try {
        if (top && top.document) isTopAccessible = true;
      } catch (e) {
      }
    }
    ...
    window.addEventListener('message', function (event) {
      ...
      if (typeof data.alpha === 'undefined') return;
      handleDeviceOrientation(data);
    });

    // Register attributes
    plugin.registerattribute("available", false, function (arg) {
    }, function () {
      return isDeviceAvailable;
    });
    plugin.registerattribute("enabled", true, function (arg) {
      stringToBoolean(arg) ? enable() : disable()
    }, function () {
      return isEnabled;
    });
    plugin.registerattribute("velastic", 0, function (arg) {
      vElasticity = Math.max(Math.min(Number(arg), 1), 0);
    }, function () {
      return vElasticity;
    });
    plugin.registerattribute("vrelative", false, function (arg) {
      isVRelative = stringToBoolean(arg);
    }, function () {
      return isVRelative;
    });
    plugin.registerattribute("camroll", false, function (arg) {
      isCamRoll = stringToBoolean(arg);
    }, function () {
      return isCamRoll;
    });
    plugin.registerattribute("friction", 0.5, function (arg) {
      friction = Math.max(Math.min(Number(arg), 1), 0);
    }, function () {
      return friction;
    });
  ...
  function stringToBoolean (value) {
    return (String("yesontrue1").indexOf(String(value).toLowerCase()) >= 0);
  };	// boolean cast helper function
}
